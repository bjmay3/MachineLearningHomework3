# Linear Discriminant Analysis (Node Flooding)
...
# Part B:  Run Linear Discriminant Analysis (LDA) algorithm

# Scale the independent variobles
sc = StandardScaler()
X = sc.fit_transform(X)

# Apply LDA to the independent variables
...
# Use three components and recompute LDA; apply to data
lda = LDA(n_components = 2)
X_new = lda.fit_transform(X, Y)

# Redo the ANN classifier on the dataset & cross-validate
# 3 hidden layers each with 12 nodes per layer
classifier = MLPClassifier(hidden_layer_sizes = (12, 12, 12), max_iter = 500, random_state=1)
classifier.fit(X_new, Y)
scores = cross_val_score(classifier, X_new, Y, cv=4)
print('Mean = ', np.mean(scores))
# Based on cross-validation, should predict with around 79% accuracy
